...
using KeywordInterface = Adapter::DBInterface<KeywordRecord>;
...
TEMPLATE_TEST_CASE("Test the Keyword interface of the Adapter and the Backend", "[keywords][adapter][backend]", DatabaseFactory, BackendFactory) {
	...
	SECTION( "Keyword with 'id' should be aquired after insertion (with getKeyword)", "[keywords][database]" ) {
		...
		CHECK( keyIF->getEntry(keyId).getKeyword() == keyword1 );
	...
	SECTION( "Inserting a Keyword twice (modulo capitalisation) should throw PhotoLibrary::constraint_error", "[keywords][database]" ) {
	...
	SECTION( "Reqesting keyword from empty database or wrong id should throw runtime_error (excluding internal keywords)", "[keywords][database]" ) {
	...
	SECTION( "Keyword should have the parent it was given", "[keywords][database]" ) {
		KeywordRecord key1(0, KeywordRecord::Options::NONE, "some key");
		int keyId;
		REQUIRE_NOTHROW(keyIF->newEntry(key1));
		REQUIRE_NOTHROW(keyId = keyIF->getID(key1));
	...
	SECTION( "Keyword should have the new parent after is was changed", "[keywords][database]" ) {
		KeywordRecord key1(0, KeywordRecord::Options::NONE, "some key");
		int keyId;
		REQUIRE_NOTHROW(keyIF->newEntry(key1));
		REQUIRE_NOTHROW(keyId = keyIF->getID(key1));
	...
	SECTION( "Vector of children should contain the children and only the children", "[keywords][database]" ) {
	...
	SECTION( "Don't hand unchecked userinput to database", "[keywords][database]" ) {
		using Catch::Matchers::VectorContains;
	...
	SECTION( "Keywords should be in database afer being added using the Keyword struct", "[keywords][database]" ) {
	...
	SECTION( "Test updating a keyword", "[keywords][database]" ) {
		//Initilaise
		Keyword animal(0, Keyword::Options::INCLUDE_ON_EXPORT|Keyword::Options::INCLUDE_SYNONYMS_ON_EXPORT, "Animal", "Animals");
		int animal_id;
		db.getKeywordInterface()->newEntry(animal);
		...
		SECTION( "After updating a keyword it should contain the new information (using the Keyword struct)", "[keywords][database]" ) {
		...
		SECTION( "Moving or renaming a keyword s.t. two identical keywords (modulo capitalisation) have the same parent should throw PhotoLibrary::constraint_error (using the Keyword struct)", "[keywords][database]" ) {
...
//		TEST_CASE( "Setting child or self as parent should throw runtime_error (using the Keyword struct)", "[keywords][database]" ) {
...
//		}
	...
	SECTION( "After a keyword was deleted it should not be in the database anymore, nor should its children be. Other keywords should not be affected.", "[keywords][database]" ) {
		//Initilaise
		Keyword animal(0, Keyword::Options::INCLUDE_ON_EXPORT | Keyword::Options::INCLUDE_SYNONYMS_ON_EXPORT, "Animal", "Animals");
		int animal_id;
		db.getKeywordInterface()->newEntry(animal);
		...
		//delete
		SECTION("Delete leaves on defferent levels.") {
		...
		SECTION("Children should be deleted as well (test 1).") {
			db.getKeywordInterface()->deleteEntry(elephant_id);
			CHECK_THROWS_AS(db.getKeywordInterface()->getEntry(elephant_id), std::runtime_error);
			CHECK_THROWS_AS(db.getKeywordInterface()->getEntry(asian_elephant_id), std::runtime_error);
			CHECK_THROWS_AS(db.getKeywordInterface()->getEntry(african_elephant_id), std::runtime_error);
		...
		SECTION("Children should be deleted as well (test 2).") {
			db.getKeywordInterface()->deleteEntry(animal_id);
			CHECK_THROWS_AS(db.getKeywordInterface()->getEntry(animal_id), std::runtime_error);
			CHECK_THROWS_AS(db.getKeywordInterface()->getEntry(giraffe_id), std::runtime_error);
			CHECK_THROWS_AS(db.getKeywordInterface()->getEntry(elephant_id), std::runtime_error);
	...
	SECTION( "root keyword can't be delted", "[keywords][database]" ) {
